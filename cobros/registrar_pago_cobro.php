...
$cuentaCobrarId = (int)($input['cuenta_cobrar_id'] ?? 0);
...
$metodosValidos = ['efectivo', 'transferencia', 'pago_movil', 'cheque', 'tarjeta', 'deposito'];
...
$stmt = $pdo->prepare("SELECT id, cliente_id, saldo_pendiente, estado FROM cuentas_cobrar WHERE id = :id");
$stmt->execute([':id' => $cuentaCobrarId]);
$cuenta = $stmt->fetch();

if (!$cuenta) {
    errorResponse('Cuenta por cobrar no encontrada', 404);
}
...
try {
    ...
    $stmtPago = $pdo->prepare("
        INSERT INTO pagos_cobro (cuenta_cobrar_id, monto, metodo_pago, referencia, fecha_pago, usuario_id, notas)
        VALUES (:cuenta_cobrar_id, :monto, :metodo_pago, :referencia, :fecha_pago, :usuario_id, :notas)
    ");
    $stmtPago->execute([
    ...
    $stmtUpdate = $pdo->prepare("UPDATE cuentas_cobrar SET saldo_pendiente = :saldo, estado = :estado WHERE id = :id");
    $stmtUpdate->execute([
        ':saldo' => $nuevoSaldo,
        ':estado' => $nuevoEstado,
        ':id' => $cuentaCobrarId,
    ...
    if ($nuevoEstado === 'pagada') {
        $stmtCliente = $pdo->prepare("SELECT estado FROM clientes WHERE id = :id");
        $stmtCliente->execute([':id' => $cuenta['cliente_id']]);
        $cliente = $stmtCliente->fetch();

        if ($cliente && $cliente['estado'] === 'moroso') {
            $stmtUpdateCliente = $pdo->prepare("UPDATE clientes SET estado = 'activo' WHERE id = :id");
            $stmtUpdateCliente->execute([':id' => $cuenta['cliente_id']]);
        }
    }
    ...
    auditoriaRegistrar(
        ...
        'cobros',
        "Pago de $monto registrado en cuenta #$cuentaCobrarId ($nuevoEstado)"
    );

    require_once __DIR__ . '/../telegram/notificar_cobro.php';
    telegramNotificarPagoCobro($pdo, $cuentaCobrarId, $monto, $metodoPago, $nuevoEstado);
    ...
    $pdo->rollBack();
    error_log("Error registrar_pago_cobro: " . $e->getMessage());
