...
try {
    ...
    if ($action === 'generar_secreto') {
        ...
        $isValid = verificarTOTP($secret, $code);
...
function verificarTOTP(string $secret, string $code): bool {
    $code = trim($code);
    ...
    $stmtBackup = $GLOBALS['pdo']->prepare("SELECT 2fa_backup_codes FROM admin_users WHERE id = ?");
    ...
    if ($user && !empty($user['2fa_backup_codes'])) {
        ...
        if (is_array($codes)) {
            ...
            if ($idx !== false) {
                unset($codes[$idx]);
                $stmtUpdate = $GLOBALS['pdo']->prepare("UPDATE admin_users SET 2fa_backup_codes = ? WHERE id = ?");
                $stmtUpdate->execute([json_encode(array_values($codes)), $_SESSION['user_id']]);
                return true;
            }
        }
    }
