...
if ($tabla_origen !== 'admin_users') {
    ...
    if ($tabla_origen === 'users') {
        session_destroy();
        header('Location: /proyecto/interfaz usuario/login.html?error=cliente_accediendo_admin');
        exit;
    }
    
    header('Location: /proyecto/interfaz usuario/login.html?error=invalid_access');
    exit;
}
...
if (!isset($_SESSION['es_admin']) || $_SESSION['es_admin'] !== true) {
    ...
    header('Location: /proyecto/interfaz usuario/login.html?error=not_admin');
    exit;
}
...
if (!in_array(strtolower($user_rol), $roles_admin)) {
    ...
    header('Location: /proyecto/interfaz usuario/login.html?error=invalid_role');
    exit;
}
...
try {
    ...
    if (!$admin_user || $admin_user['activo'] == 0) {
        ...
        session_destroy();
        header('Location: /proyecto/interfaz usuario/login.html?error=invalid_admin');
        exit;
    }
    ...
    header('Location: /proyecto/interfaz usuario/login.html?error=db_error');
    exit;
}
...
<body>
    ...
    try {
        const response = await fetch('/proyecto/usuarios/obtener usuario.php', { credentials: 'include' });
        if(response.ok) {
...
function renderCompras() {
    ...
    console.log('✅ Renderizado de compras completado,', comprasData.length, 'filas');
        ...
        async function cargarPedidos() {
            ...
            try {
                const response = await fetch('/proyecto/proceso compra/obtener_todos_los_pedidos.php', { credentials: 'include' });
                if(response.ok) {
                    const data = await response.json();
        ...
        async function cargarHistorialCompras() {
            ...
            try {
                ...
                let url = '/proyecto/proceso%20compra/obtener_todos_los_pedidos.php';
                const params = [];
...
function renderAuditoria() {
    ...
    console.log('✅ Auditoría renderizada,', auditoriaData.length, 'filas');
        ...
        function editarUsuario(id) { window.location.href = `/proyecto/admin/editar usuario.php?id=${id}`; }
        ...
        function editarPedido(pedidoId) { window.location.href = `/proyecto/proceso compra/editar_pedido.php?id=${pedidoId}`; }
        ...
        async function verDetallePedido(pedidoId) {
            ...
            try {
                const response = await fetch(`/proyecto/proceso compra/obtener_detalles_pedido.php?id=${pedidoId}`, { credentials: 'include' });
                if(response.ok) {
                    const data = await response.json();
                    if(data.success) {
    ...
    try {
        ...
        if(data.success) { 
            ...
            setTimeout(() => { 
                if(confirm('Contraseña cambiada correctamente. ¿Deseas iniciar sesión?')) 
                    window.location.href = '/proyecto/interfaz usuario/login.html'; 
            }, 1000); 
        } else { 
        ...
        async function eliminarCuenta() {
            ...
            try {
                ...
                const data = await response.json();
                if(data.success) { mostrarNotificacion('Cuenta eliminada. Cerrando sesión...', 'success'); setTimeout(() => { window.location.href = '/proyecto/interfaz usuario/login.html'; }, 2000); }
        ...
        function cerrarSesion() { if(confirm('¿Cerrar sesión?')) fetch('/proyecto/usuarios/cerrar_sesion.php', { method: 'POST', credentials: 'include' }).finally(() => { window.location.href = '/proyecto/interfaz usuario/login.html'; }); }
