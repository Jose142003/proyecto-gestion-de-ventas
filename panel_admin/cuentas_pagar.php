...
<title>Cuentas por Pagar - PIC</title>
...
<div class="sidebar" style="position:relative"><button class="mobile-menu-toggle" onclick="toggleMobileMenu()" style="position:absolute;top:15px;right:-45px;z-index:1001;background:var(--sidebar-bg,#1a1f3a);color:#fff;border:none;font-size:1.3rem;padding:8px 12px;border-radius:0 8px 8px 0;cursor:pointer;display:none"><i class="fas fa-bars"></i></button>
    ...
    <nav class="menu">
        ...
        <div class="menu-item active"><i class="fas fa-file-invoice"></i> <span data-i18n="cuentas_pagar">Cuentas Pagar</span></div>
...
<div class="main-content">
    <div class="header"><h2 data-i18n="cuentas_pagar_title"><i class="fas fa-file-invoice"></i> Cuentas por Pagar</h2><div><span id="currentDate"></span></div></div>
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-4"><input type="text" class="form-control" id="filtroProveedor" placeholder="Buscar proveedor..." data-i18n-placeholder="buscar"></div>
    ...
    <div class="table-container">
        ...
        <div class="table-content">
            <table class="data-table">
                <thead><tr><th>#</th><th>Proveedor</th><th>RUC</th><th>Documento</th><th>Monto Original</th><th>Saldo</th><th>Vencimiento</th><th>Días Venc.</th><th>0-30</th><th>31-60</th><th>61-90</th><th>90+</th><th data-i18n="estado">Estado</th><th data-i18n="acciones">Acciones</th></tr></thead>
...
<div class="modal fade" id="pagoModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-dark text-white"><h5 class="modal-title">Registrar Pago a Proveedor</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
            <form id="pagoForm">
                <div class="modal-body">
                    <input type="hidden" id="cuentaId">
                    <div class="mb-3"><label class="form-label">Proveedor</label><p class="form-control-plaintext" id="pagoProveedor"></p></div>
                    ...
                    <div class="mb-3"><label class="form-label">Método de Pago *</label><select class="form-select" id="pagoMetodo" required><option value="" data-i18n="seleccionar">Seleccionar...</option><option value="efectivo" data-i18n="efectivo">Efectivo</option><option value="transferencia" data-i18n="transferencia">Transferencia</option><option value="cheque">Cheque</option><option value="deposito">Depósito</option></select></div>
                    <div class="mb-3"><label class="form-label">Referencia</label><input type="text" class="form-control" id="pagoReferencia"></div>
                </div>
                <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal" data-i18n="cancelar">Cancelar</button><button type="submit" class="btn btn-success" id="btnRegistrarPago"><i class="fas fa-check"></i> Registrar Pago</button></div>
            </form>
        </div>
    </div>
</div>
...
function cargarResumen() {
    api(`${BASE}/pagos/obtener_resumen_pagos.php`).then(r => {
        if (!r.success) return;
        const d = r.data;
...
function cargarCuentas() {
    ...
    api(`${BASE}/pagos/obtener_cuentas_pagar.php?${params}`).then(r => {
        if (!r.success) { $('#cuentasBody').html(`<tr><td colspan="14" class="text-center text-danger">${r.message}</td></tr>`); return; }
        if (!r.data.length) { $('#cuentasBody').html('<tr><td colspan="14" class="text-center">' + __('sin_datos') + '</td></tr>'); return; }
        let h = '';
        r.data.forEach(c => {
            const est = c.estado;
            const badge = est === 'pagada' ? 'success' : est === 'vencida' ? 'danger' : est === 'parcial' ? 'warning' : 'secondary';
            h += `<tr>
                <td>${c.id}</td><td>${esc(c.proveedor_nombre)}</td><td>${esc(c.proveedor_ruc||'')}</td>
                ...
                <td>${est !== 'pagada' && est !== 'anulada' ? `<button class="btn-action btn-pay" onclick="abrirPago(${c.id},'${esc(c.proveedor_nombre)}',${c.saldo_pendiente})"><i class="fas fa-dollar-sign"></i> Pagar</button>` : ''}</td>
            </tr>`;
        });
        $('#cuentasBody').html(h);
...
function abrirPago(id, proveedor, saldo) {
    $('#cuentaId').val(id);
    $('#pagoProveedor').text(proveedor);
...
$('#pagoForm').submit(function(e) {
    e.preventDefault();
    const data = {
        cuenta_pagar_id: parseInt($('#cuentaId').val()),
        monto: parseFloat($('#pagoMonto').val()),
        metodo_pago: $('#pagoMetodo').val(),
        referencia: $('#pagoReferencia').val().trim()
    };
    if (!data.monto || data.monto <= 0) { notif('Ingrese un monto válido', 'danger'); return; }
    if (!data.metodo_pago) { notif('Seleccione método de pago', 'danger'); return; }
    $('#btnRegistrarPago').prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Procesando...');
    api(`${BASE}/pagos/registrar_pago_proveedor.php`, { method: 'POST', body: JSON.stringify(data) }).then(r => {
        if (!r.success) { notif(r.message, 'danger'); $('#btnRegistrarPago').prop('disabled', false).html('<i class="fas fa-check"></i> Registrar Pago'); return; }
        notif(__('registro_guardado'));
        bootstrap.Modal.getInstance('#pagoModal').hide();
        cargarResumen();
        cargarCuentas();
        $('#btnRegistrarPago').prop('disabled', false).html('<i class="fas fa-check"></i> Registrar Pago');
    }).catch(() => { notif(__('error_conexion'), 'danger'); $('#btnRegistrarPago').prop('disabled', false).html('<i class="fas fa-check"></i> Registrar Pago'); });
});
