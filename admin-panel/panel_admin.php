...
if ($tabla_origen !== 'admin_users') {
    ...
    if ($tabla_origen === 'users') {
        session_destroy();
        header('Location: /proyecto/usuario/login.html?error=cliente_accediendo_admin');
        exit;
    }
    
    header('Location: /proyecto/usuario/login.html?error=invalid_access');
    exit;
}
...
if (!isset($_SESSION['es_admin']) || $_SESSION['es_admin'] !== true) {
    ...
    header('Location: /proyecto/usuario/login.html?error=not_admin');
    exit;
}
...
if (!in_array(strtolower($user_rol), $roles_admin)) {
    ...
    header('Location: /proyecto/usuario/login.html?error=invalid_role');
    exit;
}
...
try {
    ...
    if (!$admin_user || $admin_user['activo'] == 0) {
        ...
        session_destroy();
        header('Location: /proyecto/usuario/login.html?error=invalid_admin');
        exit;
    }
    ...
    header('Location: /proyecto/usuario/login.html?error=db_error');
    exit;
}
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* ========== RESPONSIVE DESIGN PARA admin-panel ========== */
    ...
    try {
        ...
        if(data.success) { 
            ...
            setTimeout(() => { 
                if(confirm('Contraseña cambiada correctamente. ¿Deseas iniciar sesión?')) 
                    window.location.href = '/proyecto/usuario/login.html'; 
            }, 1000); 
        } else { 
        ...
        async function eliminarCuenta() {
            ...
            try {
                ...
                const data = await response.json();
                if(data.success) { mostrarNotificacion('Cuenta eliminada. Cerrando sesión...', 'success'); setTimeout(() => { window.location.href = '/proyecto/usuario/login.html'; }, 2000); }
        ...
        function cerrarSesion() { if(confirm('¿Cerrar sesión?')) fetch('/proyecto/usuarios/cerrar_sesion.php', { method: 'POST', credentials: 'include' }).finally(() => { window.location.href = '/proyecto/usuario/login.html'; }); }
